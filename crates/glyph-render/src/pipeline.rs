...
/// Renders textured quads for raster images. Uses the same `TextVertex` layout
/// (pos + uv + color) but samples an RGBA8 texture instead of an R8 glyph atlas.
...
impl ImagePipeline {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat, width: f32, height: f32) -> Self {
        ...
        let screen_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("img_screen_uniform"),
            contents: bytemuck::cast_slice(&[ScreenUniform { size: [width, height] }]),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });

        let screen_bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("img_screen_bgl"),
            entries: &[wgpu::BindGroupLayoutEntry {
                binding: 0,
                visibility: wgpu::ShaderStages::VERTEX,
                ty: wgpu::BindingType::Buffer {
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("img_layout"),
            bind_group_layouts: &[&screen_bgl, &image_bgl],
            push_constant_ranges: &[],
        });
        ...
        Self { pipeline, screen_bg, screen_buf, image_bgl }
    }
