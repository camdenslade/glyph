...
/// use glyph_hot::guest::glyph_guest;
///
/// struct MyApp { /* Signal handles from GlyphSignalTable */ }
///
/// impl MyApp {
///     pub fn new(signals: &glyph_hot::abi::GlyphSignalTable) -> Self { ... }
/// }
///
/// impl glyph_core::Component for MyApp {
///     fn render(&self, theme: &glyph_core::Theme) -> glyph_core::View { ... }
/// }
///
/// glyph_guest!(MyApp);
/// ```
...
/// - `glyph_hot_free_node` — frees a `CViewDesc` node allocated by the guest
/// - `glyph_hot_free_str`  — frees a C string allocated by the guest
///
/// `glyph_build_view` is the hot path called every frame. It converts the
/// `*const CTheme` into a `glyph_core::Theme`, calls `render`, then serializes
